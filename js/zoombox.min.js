...
this.markActiveImage(this.index);this.zoomboxPreview.style.transform=`translateX(-${((this.currIndex/this.images.length)*100)}%)`;window.addEventListener('keydown',this.addEvents);this.zoomboxBox.addEventListener('wheel',this.zoomin,{passive:!0})}
...
closeZoombox=()=>{this.zoomboxBox.style.opacity='0';document.body.style.overflow='auto';this.checkPlay(!0);setTimeout(()=>{this.zoomboxBox.style.display='none';this.zoomboxBox.style.opacity='1';let last=this.zoomboxPreview.lastElementChild;while(last){this.zoomboxPreview.removeChild(last);last=this.zoomboxPreview.lastElementChild}},200)
window.removeEventListener('keydown',this.addEvents);this.zoomboxBox.removeEventListener('touchstart',this.touchStart,{passive:!0});this.zoomboxBox.removeEventListener('touchend',this.touchEnd,{passive:!0})}
...
touchControl=()=>{this.zoomboxBox.addEventListener('touchstart',this.touchStart,{passive:!0});this.zoomboxBox.addEventListener('touchend',this.touchEnd,{passive:!0})}
...
var zoombox=new zoomboxImages;const imageGroup=document.querySelectorAll('[zoombox]');var groupName=[];imageGroup.forEach((image)=>{let att=image.getAttribute('zoombox');if(att==""){image.setAttribute('zoombox','default');att='default'}
if(!(groupName.includes(att)))
groupName.push(att)})
function zoomBoxExecute(){groupName.forEach((group)=>{var images=document.querySelectorAll(`[zoombox=${group}]`);images.forEach((img,index)=>{img.onclick=()=>{var groupName=img.getAttribute('zoombox');zoombox=new zoomboxImages(`[zoombox=${groupName}]`,img.src,img.getAttribute('src2'),img.alt,index);zoombox.execute();document.querySelector('.zoombox-close').src=`${clientRootDir}images/close.png`;document.querySelector('.zoombox-zoom-in').src=`${clientRootDir}images/zoom-in.svg`;document.querySelector('.zoombox-zoom-out').src=`${clientRootDir}images/zoom-out.svg`;document.querySelector('.zoombox-next').src=`${clientRootDir}images/next.png`;document.querySelector('.zoombox-prev').src=`${clientRootDir}images/prev.png`;document.querySelector('.zoombox-play').src=`${clientRootDir}images/play.png`}})})}
